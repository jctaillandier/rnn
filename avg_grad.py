...
def run_epoch(model, data):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        grads = []
        for i in range(len(model.hidden_states)):
            grad = torch.autograd.grad(loss, model.hidden_states[i][1:], retain_graph=True)
            grads.append(grad)

        stacked_grads = []
        for s in range(len(grads[0])):
            stacked_grads.append(torch.stack([grads[j][s] for j in range(len(grads))]))

        ts_grads = [grad.norm() for grad in stacked_grads]
        plt.plot(ts_grads)
